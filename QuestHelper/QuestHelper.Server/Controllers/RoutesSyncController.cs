...
namespace QuestHelper.Server.Controllers
{
    ...
    public class RoutesSyncController : Controller
    {
        [HttpPost]
        public IActionResult Post([FromBody]SyncObjectVersions syncObject)
        {
            string userId = "0";
            SyncObjectVersions report = new SyncObjectVersions();
            using (var db = new ServerDbContext())
            {
                ...
                foreach(var routeVersion in syncObject.Statuses)
                {
                    ...
                    if (dbRoute != null)
                    {
                        ...
                        if(dbRoute.Version != routeVersion.Version)
                            report.Statuses.Add(new SyncObjectVersions.ObjectStatus { ObjectId = dbRoute.RouteId, Version = dbRoute.Version});
                        //если версия одна, не возвращаем
                    } else
                    {
                        //если маршрута нет, вернем инфу о том, что надо его запушить на сервер
                        report.Statuses.Add(new SyncObjectVersions.ObjectStatus { ObjectId = routeVersion.ObjectId, Version = 0 });
                    }
                }
                ...
                {
                    report.Statuses.Add(new SyncObjectVersions.ObjectStatus { ObjectId = newRoute.RouteId, Version = newRoute.Version });
                }
            }

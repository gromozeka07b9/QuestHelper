...
namespace QuestHelper.Model
{
    public class ViewRoute : ISaveable, INotifyPropertyChanged
    {
        ...
        public bool MakeFromAutoRoute(AutoGeneratedRouted autoGeneratedRoute, string creatorId)
        {
            CreatorId = creatorId;
            CreateDate = DateTimeOffset.Now;
            Description = "Автоматически созданный маршрут";
            Name = autoGeneratedRoute.Name;
            if (Save())
            {
                foreach(var point in autoGeneratedRoute.Points)
                {
                    if (!point.IsDeleted)
                    {
                        var vroutePoint = new ViewRoutePoint(Id, string.Empty);
                        vroutePoint.Name = point.Name;
                        vroutePoint.Longitude = point.Longitude;
                        vroutePoint.Latitude = point.Latitude;
                        vroutePoint.Version++;
                        if (vroutePoint.Save())
                        {
                            foreach(var img in point.Images)
                            {
                                vroutePoint.AddMediaItem("111", MediaObjectTypeEnum.Image);
                                vroutePoint.Save();
                            }
                        }
                    }

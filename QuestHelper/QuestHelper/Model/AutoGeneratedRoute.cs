...
namespace QuestHelper.Model
{
    internal class AutoGeneratedRoute
    {
        ...
        internal void Build()
        {
            ...
            var dictPhotos = new Dictionary<(GpsCoordinates, DateTime), GalleryImage>();
            foreach (var image in SourceGalleryImages)
            {
                var resultImage = imageManager.GetPhoto(image.ImagePath);
                dictPhotos.Add((resultImage.imageGpsCoordinates, image.CreateDate), image);
            }
            var sortedByTime = dictPhotos.OrderBy(x => x.Key.Item2);
            ...
            foreach (var image in sortedByTime)
            {
                ...
                {
                    AutoGeneratedPoint point = new AutoGeneratedPoint() { Name = (number+1).ToString(), CreateDate = date, Longitude = number, Latitude = number };//Добавить широту и долготу
